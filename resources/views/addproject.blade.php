        ...
        <div class="container-fluid py-4">
            ...
            <form id="projectForm">
              ...
              <div id="collaborators">
                  <div class="collaborator mb-3">
                      <label for="adduser" class="form-label">Usuario</label>
                      <select class="form-select user-select" id="adduser">
                          <option value="" disabled selected>Selecciona el usuario</option>
                          <!-- Opciones se cargarán dinámicamente -->
                      </select>
                      <label for="rol" class="form-label">Rol</label>
                      <input type="text" class="form-control role-input" placeholder="Rol del usuario">
                      <button type="button" class="btn btn-danger remove-collaborator mt-2">Eliminar</button>
                  </div>
              </div>
              <button type="button" id="addCollaborator" class="btn btn-primary mb-3">Agregar otro colaborador</button>
          
              <!-- Botón de Cancelar y Guardar -->
              <div class="d-flex justify-content-end">
                  <a class="btn btn-outline-dark me-2" href="/home">Cancelar</a>
                  <button type="submit" class="btn btn-dark">Guardar</button>
              </div>
          </form>
          
          <script>
            // Cargar usuarios dinámicamente en el select
            document.addEventListener('DOMContentLoaded', async () => {
                const userSelect = document.getElementById('adduser');
                const currentUserId = {{ auth()->id() }}; // ID del usuario que está logueado
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        users.forEach(user => {
                            // Evitar que el usuario logueado se seleccione a sí mismo
            ...
            // Manejar dinámicamente los colaboradores
            document.getElementById("addCollaborator").addEventListener("click", () => {
                ...
                const existingUserIds = Array.from(document.querySelectorAll('.user-select'))
                    .map(select => select.value); // IDs de los colaboradores ya agregados
        
                const newCollaborator = `
                    <div class="collaborator mb-3">
                        <label for="adduser" class="form-label">Usuario</label>
                        <select class="form-select user-select" required>
                            ${Array.from(document.querySelectorAll("#adduser option"))
                                .map(option => {
                                    // Evitar agregar usuarios ya seleccionados
                                    if (existingUserIds.includes(option.value)) {
                                        return ''; // No incluir opciones ya seleccionadas
                                    }
                        ...
                        <label for="rol" class="form-label">Rol</label>
                        <input type="text" class="form-control role-input" placeholder="Rol del usuario" required>
                        <button type="button" class="btn btn-danger remove-collaborator mt-2">Eliminar</button>
                    </div>`;
                collaboratorsDiv.insertAdjacentHTML("beforeend", newCollaborator);
            });
        
            // Eliminar un colaborador
            document.getElementById("collaborators").addEventListener("click", (event) => {
            ...
            projectForm.addEventListener("submit", (event) => {
                ...
                const collaborators = Array.from(document.querySelectorAll(".collaborator")).map(collab => {
                    const userSelect = collab.querySelector(".user-select");
                    const roleInput = collab.querySelector(".role-input");
        
                    // Verificar que el colaborador y el rol no estén vacíos
                    if (!userSelect.value || !roleInput.value.trim()) {
                        alert('Por favor, ingrese un colaborador y un rol para todos los campos.');
                        throw new Error('Colaborador y rol requeridos');
                    }
                ...
                const formData = {
                    ...
                    description,
                    status,
                    created_by: {{ auth()->id() }}, // ID dinámico del usuario logueado
                    collaborators
                };
