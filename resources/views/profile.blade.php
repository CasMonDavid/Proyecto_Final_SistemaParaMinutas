                    ...
                    <div class="d-flex justify-content-between align-items-end">
                        <div class="d-flex align-items-center">
                            <div class="avatar avatar-xl position-relative me-3">
                                ...
                                <img src="{{ Auth::user() && Auth::user()->profile_picture ? asset('storage/' . Auth::user()->profile_picture) : '../assets/img/bruce-mars.jpg' }}"
                    ...
                    <div class="card-body p-3">
                        <div class="overflow-auto">
                            <div class="d-flex flex-nowrap">
                                ...
                                <div id="project-container" style="display: flex; flex-wrap: wrap; gap: 20px;"></div>

                                <script>
                                    document.addEventListener("DOMContentLoaded", () => {
                                        const projectContainer = document.getElementById("project-container");

                                        // Verificar que el contenedor existe antes de manipularlo
                                        if (projectContainer !== null) {

                                            // ID del usuario que inició sesión (puedes reemplazar esto dinámicamente desde tu backend o frontend)
                                            const loggedInUserId = {{ auth()->id() }}; // ID del usuario autenticado

                                            // Realizar solicitud AJAX al endpoint de los proyectos
                                            fetch('proyectos')
                                                .then(response => response.json())
                                                .then(projects => {
                                                    // Filtrar los proyectos donde el campo 'created_by' coincide con el ID del usuario autenticado
                                                    const userProjectsFiltered = projects.filter(project => project.created_by ===
                                                        loggedInUserId);

                                                    // Renderizar los proyectos filtrados
                                                    userProjectsFiltered.forEach(project => {
                                                        const card = document.createElement('div');
                                                        card.className = "col-xl-3 col-md-6 mb-xl-0 mb-4 me-4";
                                                        card.innerHTML = `
                                                        <div class="card card-blog card-plain">
                                                          <div class="position-relative">
                                                            <a class="d-block" href="/project/${project.id}">
                                                              <img src="${project.image_url || '../assets/img/home-decor-3.jpg'}" alt="img-blur-shadow" class="img-fluid shadow border-radius-md">
                                                            </a>
                                                          </div>
                                                          <div class="card-body px-1 pb-0">
                                                            ...
                                                            <p class="mb-4 text-sm">
                                                              ${project.description || 'Descripción no disponible'}  <!-- Descripción del proyecto -->
                                                            </p>

                                                            <div class="d-flex align-items-center justify-content-between">
                                                              <a type="button" href="/project/1" class="btn btn-outline-primary btn-sm mb-0">Ver proyecto</a>
                                                              <div class="avatar-group mt-2" style="display: flex; gap: 5px; flex-wrap: nowrap; overflow-x: auto;">
                                                                ${project.collaborators.map(collaborator => `
                                                            <a href="javascript:;" class="avatar avatar-xs rounded-circle" data-bs-toggle="tooltip" data-bs-placement="bottom" title="${collaborator.name}">
                                                              <img alt="Image placeholder" src="${collaborator.image || '../assets/img/team-4.jpg'}">
                                                            </a>
                                                          `).join('')}
                                                              </div>
                                                            </div>
